...
def neededSpeedReference (robot):  
    if (robot.collisionTime >= robot.upperTimeThreshold):
        return (robot.absolutMaxSpeed)
...
def fillAndUpdatePositionListRobot(positions_per_second, positions_saved, xList, yList, robot):
    ...
    while True:
        ...
        try:
            ...
            xList.append(xyValues[0]) 
            yList.append(xyValues[1]) 
            ...
            if len(xList) != len(yList): # Handles the case where xPath and yPath have a different number of elements, though I don't see how that could happen
                print("ERROR in positional tracking. Resetting position list") 
                xList.clear()
                yList.clear()
...
def timeToCollision (predictedX1, predictedY1, predictedX2, predictedY2, timeMargin, timeDelta, minimumEuclideanDistance):
    index = 0 
...
def calculatePath (x_or_ypoints, timeDelta):
    ...
    polynomial_features = polynomial.fit_transform(timeArray)
