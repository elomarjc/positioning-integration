...
class robot_controller():
    ...
    def __init__(self, control_panel: tkinter.Frame, robot_records: dict,
        ...
        self.position_a_photo = ImageTk.PhotoImage(tools.map.new_letter_a_icon)
        self.position_b_photo = ImageTk.PhotoImage(tools.map.new_letter_b_icon)
    ...
    def initialize_control_panel(self):
        ...
        # button about the "move to a" function
        ...
        # button about the "move to b" function
        ...
        base_rely = base_rely + 0.04 + spacing
    ...
    def draw_a(self):
        ...
        tools.map.show_position_icon(
            self.map_canvas, tools.basewindow.proportion, robot_x, robot_y,
            self.canvas_tag, self.position_a_photo, "",
            tools.map.convert_robot,
            tools.map.convert_robot_position_for_unification, 0, 10, False)

    # draw the position B on the map
    def draw_b(self):
        x = self.b_x
        y = self.b_y
        print("Preview the coordinates: ({}, {}).".format(x, y))
        ...
        tools.map.show_position_icon(
            self.map_canvas, tools.basewindow.proportion, robot_x, robot_y,
            self.canvas_tag, self.position_b_photo, "",
            tools.map.convert_robot,
            tools.map.convert_robot_position_for_unification, 0, 10, False)

    # move the selected robot to the position A
    def move_to_a(self):
    ...
    # move the selected robot to the position B
    def move_to_b(self):
        robot_id = self.id_option_value.get()
        robot_record_key = tools.tools.robot_canvas_tag(robot_id)
        robot_ip = self.robot_records[robot_record_key].ip
