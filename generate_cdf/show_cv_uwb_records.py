...
def show_positions():
    record_file_name = "generate_cdf/cvexperiment20220817/every_100ms_uwb_cv_dist.csv"
    ...
    while True:
        ...
        cv_x = records[frame_index][1]
        cv_y = records[frame_index][3]
        uwb_x = records[frame_index][5]
        uwb_y = records[frame_index][7]
        dist = math.sqrt(
            pow((float(cv_x) - float(uwb_x)), 2) +
            pow((float(cv_y) - float(uwb_y)), 2))

        print("cv: ({}, {}), uwb: ({}, {}), distance: {}m".format(
            cv_x, cv_y, uwb_x, uwb_y, dist))
        record_text.set(
            "Current record:\nCV: ({:.2f}, {:.2f})\nUWB: ({:.2f}, {:.2f})\nDistance: {:.2f}m"
            .format(float(cv_x), float(cv_y), float(uwb_x), float(uwb_y),
                    dist))

        # delete the old point on the map, and draw this point
        tools.basewindow.canvas_map.delete(cv_canvas_tag)
        tools.basewindow.canvas_map.delete(uwb_canvas_tag)
        # draw the new points
        cv_x_origin, cv_y_origin = tools.map.reverse_convert_cv_position_for_unification(
            float(cv_x), float(cv_y))
        tools.map.show_position(tools.basewindow.canvas_map,
                                tools.basewindow.proportion, cv_x_origin,
                                cv_y_origin, cv_canvas_tag, cv_color, "",
